...
class getMelSpectrogram:
    ...
    # from the librosa documentation (https://librosa.org/doc/main/generated/librosa.stft.html)
    ...
    def power_mel_to_db(self, D_mel, a_squere_min, dB_ref):
        ...
        # --> values over dB_ref are set to dB_ref
        # -120 dB as the min dB value -> log_10(1e-12) -> values inside the log should be larger or equal to 1e-12
        # --> values under a_square_min are set to a_square_min

        # also tried with linear mapping, but this results in worse resolution for small values, than the dB scale mapping
        # D_mel_dB = -120 + (120/dB_ref) * np.maximum(a_square_min, np.minimum(D_mel, dB_ref)) 

        return D_mel_dB

    ########################################################
    # calculate dB-scaled mel-spectrogram
    def get_mel_spectrogram(self, audio_file_path, sampling_rate, hop_length, n_mels, f_min, f_max, a_squere_min, dB_ref):
        ...
        n_fft = self.next_power_of_2(hop_length)
